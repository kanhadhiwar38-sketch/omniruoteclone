...
// Virtual automatic-routing aliases, resolved dynamically by the SSE router from
// whichever providers/models are currently configured. Shown in their own
// "Automatic Routing" section with friendly display names, kept separate from
// saved combos above.
const AUTO_ALIAS_DISPLAY = {
  "auto": "Auto Router",
  "free-auto": "Free Auto Router",
  "bestcoding-auto": "Best Coding Auto",
  "bestreasoning-auto": "Best Reasoning Auto",
  "bestchat-auto": "Best Chat Auto",
  "bestcodingfast": "Best Coding Fast",
};
const AUTO_ALIAS_NAMES = Object.keys(AUTO_ALIAS_DISPLAY);
...
export default function ModelSelectModal({
  ...
  // Automatic routing aliases (auto, bestcoding-auto, ...) — always available,
  // not tied to a saved combo record. Hidden alongside combos when kindFilter
  // is set, since they only resolve to LLM chat models.
  const filteredAutoAliases = useMemo(() => {
    if (kindFilter || capFilter) return [];
    const query = searchQuery.trim().toLowerCase();
    if (!query) return AUTO_ALIAS_NAMES;
    return AUTO_ALIAS_NAMES.filter(
      (name) => name.toLowerCase().includes(query) || AUTO_ALIAS_DISPLAY[name].toLowerCase().includes(query)
    );
  }, [searchQuery, kindFilter, capFilter]);
  ...
  return (
    ...
    >
      ...
      <div className="max-h-[400px] overflow-y-auto space-y-3">
        ...
        {/* Automatic routing aliases - always second */}
        {filteredAutoAliases.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 mb-1.5 sticky top-0 bg-surface py-0.5">
              <span className="material-symbols-outlined text-primary text-[14px]">bolt</span>
              <span className="text-xs font-medium text-primary">Automatic Routing</span>
              <span className="text-[10px] text-text-muted">({filteredAutoAliases.length})</span>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {filteredAutoAliases.map((name) => {
                const isSelected = selectedModel === name;
                return (
                  <button
                    key={`auto-${name}`}
                    onClick={() => handleSelect({ id: name, name: AUTO_ALIAS_DISPLAY[name], value: name })}
                    title={name}
                    className={`
                      px-2 py-1 rounded-xl text-xs font-medium transition-all border hover:cursor-pointer flex items-center gap-1
                      ${isSelected
                        ? "bg-primary text-white border-primary"
                        : addedModelValues.includes(name)
                          ? "bg-primary border-primary text-white hover:bg-primary-hover"
                          : "bg-surface border-border text-text-main hover:border-primary/50 hover:bg-primary/5"
                      }
                    `}
